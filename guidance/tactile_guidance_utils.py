...
class TextureDreambooth(nn.Module):
    def __init__(
        ...
        pipe = StableDiffusionPipeline.from_pretrained(
            model_key, torch_dtype=self.dtype, safety_checker=None, requires_safety_checker=False
        ).to(device)
    ...
    def refine(self, pred_rgb,
               guidance_scale=100, steps=50, strength=0.8, normalize_mean=0.5, normalize_std=0.5
        ):
    ...
    def encode_imgs(self, imgs):
        ...
        imgs = (imgs - 0.5) / 0.5 # convert range from [0,1] to [-1, 1], same as the transforms.Normalize(0.5, 0.5) in train_dreambooth.py
