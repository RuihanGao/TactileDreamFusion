...
UNET_TARGET_MODULES = ["to_k", "to_q", "to_v", "query", "value"]  # , "ff.net.0.proj"]
# Possible variations of the target modules to finetune
# to_k to_q to_v
# attn2
# attn1.to_
# to_k to_v
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--lora_r", type=int, default=16, help="Lora rank, only used if use_lora is True")
    ...
    parser.add_argument("--add_tokens", type=int, default=0, help="Number of tokens to add to the tokenizer.")
    parser.add_argument("--new_token", type=str, default="<new1>", help="The new token to add to the tokenizer.")
...
def main(args):
    ...
    # Modify the tokenizer to add custom tokens
    print(f"Original length of tokenizer: {len(tokenizer)}")
    if args.add_tokens > 0:
        if args.new_token not in tokenizer.get_vocab():
            print(f"Adding new token: {args.new_token}")
            num_added_tokens = tokenizer.add_tokens([args.new_token])
            print(f"Added {num_added_tokens} tokens")
            # resize the model embedding
        text_encoder.resize_token_embeddings(len(tokenizer))
    print(f"New length of tokenizer: {len(tokenizer)}") # 49408 -> 49409
        ...
        if args.use_lora:
            ...
            if args.train_text_encoder:
                ...
                if args.add_tokens > 0:
                    ...
                    unwrapped_tokenizer.save_pretrained(
